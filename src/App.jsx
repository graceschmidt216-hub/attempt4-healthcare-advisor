...
const SYSTEM_PROMPT = `You are a friendly, knowledgeable health insurance advisor helping someone choose between their Aetna health plans offered through their employer (Rippling PEO 1, Inc.) for the coverage period December 2025 - November 2026. You are embedded in a plan comparison tool.

Here are the available plans:
${PLAN_CONTEXT}

Key concepts to use in your advice:
- Deductible: amount paid out of pocket before insurance covers most services
- Out-of-pocket maximum (OOP Max): the most they'll pay in a year — after this, insurance covers 100%
- Copay: flat fee per visit regardless of deductible
- Coinsurance: percentage of cost paid after meeting deductible
- EPO plans: in-network only (no out-of-network coverage except emergencies)
- PPO/POS plans: cover both in-network and out-of-network (at higher cost)
- HSA (Health Savings Account): tax-advantaged savings account only available with the $1650 HSA plan — contributions are pre-tax, roll over forever, can be invested
        ...
        <div style={{ background: "white", borderRadius: 16, boxShadow: "0 2px 8px rgba(0,0,0,0.08)", overflow: "hidden", display: "flex", flexDirection: "column", height: 580 }}>
          ...
          <div style={{ padding: "12px 16px", borderTop: "1px solid #e2e8f0", display: "flex", gap: 10 }}>
            ...
            <button onClick={sendMessage} disabled={loading || !input.trim()} style={{
              padding: "0 18px", borderRadius: 12, border: "none", background: loading || !input.trim() ? "#e2e8f0" : "#2563eb",
              color: loading || !input.trim() ? "#94a3b8" : "white", cursor: loading || !input.trim() ? "not-allowed" : "pointer",
